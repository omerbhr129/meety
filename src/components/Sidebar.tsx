  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col h-full">
        {/* Logo and Toggle */}
        <div 
          className={`flex items-center p-6 border-b border-blue-100/50 cursor-pointer transition-all duration-300 hover:bg-blue-50/50 group ${isCollapsed ? "justify-center" : "justify-between"}`}
        >
          <div className={`transition-[width,opacity] duration-500 ease-in-out overflow-hidden ${isCollapsed ? "w-0 opacity-0" : "w-full opacity-100"}`}>
            ...
            >
              <span className="text-3xl font-bold bg-gradient-to-r from-blue-600 via-purple-500 to-pink-400 bg-clip-text text-transparent transition-all duration-500 group-hover:scale-110 inline-block">
                Meety
              </span>
            </button>
          </div>
          <button 
            ...
            className="transition-all duration-300 rounded-xl p-2 hover:bg-blue-100/30 group-hover:scale-110 shrink-0 focus:outline-none active:outline-none bg-white/0 active:bg-white focus:bg-white nav-icon"
          >
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={`flex items-center justify-end gap-3 px-4 py-3 rounded-xl w-full transition-all duration-300 ease-in-out transform nav-icon ${isActive ? "bg-blue-600 text-white shadow-lg shadow-blue-200/50" : "text-gray-700 hover:bg-blue-100/50"} ${isHovered === item.id && !isActive ? "scale-105" : ""} ${isActive && isHovered === item.id ? "scale-[1.02]" : ""}`}
                  >
                    ...
                    <Icon className={`h-5 w-5 transition-all duration-300 shrink-0 ${isHovered === item.id ? "scale-110 rotate-6" : ""} ${isActive && isHovered === item.id ? "scale-110" : ""}`} />
                  </button>
                </li>
              );
            })}
            ...
            <DropdownMenuTrigger asChild>
              <button className="w-full p-2 rounded-xl hover:bg-blue-100/50 transition-all duration-300 group">
                <div className="relative w-full flex items-center transition-all duration-500 ease-in-out">
                  ...
                  <div className={`shrink-0 transition-all duration-500 ease-in-out ${isCollapsed ? "translate-x-[calc(50%-1.25rem)]" : "translate-x-0"}`}>
                    {user?.profileImage ? (
