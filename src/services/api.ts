...
export const getMeeting = async (shareableLink: string) => {
  try {
    ...
    const response = await api.get<{ meeting: Meeting }>(`/meetings/${shareableLink}`);
...
export const updateMeetingSlotStatus = async (
  ...
  try {
    console.log('Updating meeting slot status:', { meetingId, slotId, status });
...
export const bookMeeting = async (shareableLink: string, bookingData: BookMeetingDto) => {
  try {
    ...
    const response = await api.post<{ booking: Meeting['bookedSlots'][0] }>(
