...
// הגדרות CORS מורחבות
app.use(cors({
  origin: true, // אפשר גישה מכל דומיין בפרודקשן
  credentials: true,
...
app.get('/api/meetings/:id', async (req, res) => {
  try {
    const meeting = await Meeting.findById(req.params.id).populate('creator', 'fullName email');
    if (!meeting) {
...
app.post('/api/meetings/:id/book', async (req, res) => {
  try {
    ...
    let participant = await Participant.findOne({ email: attendee.email });
    ...
    if (!participant) {
      ...
      if (!participant.meetings.includes(meeting._id)) {
        participant.meetings.push(meeting._id);
      }
