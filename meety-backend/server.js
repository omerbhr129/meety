...
// Helper function to get day of week in local time
const getUTCDayOfWeek = (dateStr) => {
  // Create date object from string (will be in local time)
  const date = new Date(dateStr);
  // Get local day (0-6, where 0 is Sunday)
...
const isTimeSlotAvailable = (time, dayAvailability, duration) => {
  ...
  // If there are no time slots, the day is not available
  ...
  // Check if the requested time fits within any of the available time slots
...
// Body parser middleware - only for non-GET requests
...
app.get('/meetings/:id', async (req, res) => {
  try {
    ...
    // Try to find meeting by shareableLink first
    let meeting = await Meeting.findOne({ shareableLink: id });
    
    // If not found by shareableLink, try by _id
    if (!meeting) {
...
app.post('/participants', express.json(), async (req, res) => {
  try {
    ...
    try {
      ...
      // Make sure we return the participant in the correct format
...
app.post('/meetings/:id/book', express.json(), async (req, res) => {
  try {
    ...
    // Update participant's meetings and lastMeeting
...
app.post('/meetings', auth, express.json(), async (req, res) => {
  try {
    ...
    // Create meeting with userId as ObjectId
...
app.put('/meetings/:id', auth, express.json(), async (req, res) => {
  try {
    ...
    // Update meeting with new availability structure
...
app.get('/participants', auth, async (req, res) => {
  try {
    ...
    // First, find all meetings for the current user
    const meetings = await Meeting.find({ 
      userId: new mongoose.Types.ObjectId(req.userId),
      status: 'active'
    });
    ...
    // Find all participants who have any of these meetings
